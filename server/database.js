...
let dbUrl;
if (process.env.DATABASE_URL2 !== undefined) {
    dbUrl = process.env.DATABASE_URL2;
...
exports.cancelTransaction = function(id) {
    return db.query(
        "UPDATE cc SET status='REJECTED' WHERE id_transsaction=$1",
        [id]
    );
};
...
exports.getCountry = function({code}={}) {
    return db.query("SELECT * FROM cities WHERE prefix=$1 OR isdefault=true",[code.toUpperCase()]).then(async result => {
